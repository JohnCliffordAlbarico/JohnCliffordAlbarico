<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0e75b6&height=120&section=header" alt="header" />

<h1 align="center">John Clifford M. Albarico</h1>
<p align="center">Software Developer</p>

<p align="center">
  <a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1000&color=58A6FF&center=true&width=435&lines=Web+%C2%B7+Mobile+%C2%B7+Desktop;React+%C2%B7+React+Native+%C2%B7+Electron;Node+%2B+Express+behind+them;Supabase+%2F+Postgres+most+days" alt="typing" /></a>
</p>

<p align="center">One idea, three screens: web, mobile, and desktop from one product mindset, with backends that hold up when the network doesn't.</p>

<p align="center">
  <a href="https://github.com/JohnCliffordAlbarico"><img src="https://img.shields.io/badge/GitHub-JohnCliffordAlbarico-181717?style=flat&logo=github" alt="github" /></a>
  <img src="https://img.shields.io/badge/BSIT-Cum_Laude-blue?style=flat" alt="honors" />
</p>

### Stack

<p>
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black" alt="js" />
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white" alt="ts" />
  <img src="https://img.shields.io/badge/React-61DAFB?style=flat&logo=react&logoColor=black" alt="react" />
  <img src="https://img.shields.io/badge/React_Native-61DAFB?style=flat&logo=react&logoColor=black" alt="native" />
  <img src="https://img.shields.io/badge/Electron-47848F?style=flat&logo=electron&logoColor=white" alt="electron" />
  <img src="https://img.shields.io/badge/Node.js-339933?style=flat&logo=nodedotjs&logoColor=white" alt="node" />
  <img src="https://img.shields.io/badge/Express-000000?style=flat&logo=express&logoColor=white" alt="express" />
  <img src="https://img.shields.io/badge/Supabase-3ECF8E?style=flat&logo=supabase&logoColor=white" alt="supabase" />
  <img src="https://img.shields.io/badge/Postgres-4169E1?style=flat&logo=postgresql&logoColor=white" alt="pg" />
  <img src="https://img.shields.io/badge/SQLite-003B57?style=flat&logo=sqlite&logoColor=white" alt="sqlite" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white" alt="docker" />
  <img src="https://img.shields.io/badge/Cloudflare_R2-F38020?style=flat&logo=cloudflare&logoColor=white" alt="r2" />
</p>

JavaScript and TypeScript core. React on web, React Native on mobile, Electron on desktop, Node plus Express behind them. Supabase and Postgres most days, TursoDB and SQLite where offline or edge fits. Earlier years in Python, Java, and FastAPI. I ship with Docker, Render, Vercel, and R2.

### Builds

- **Bacaltos Healthcare System** — offline-first clinic management that syncs to the cloud. Electron shell, React care workflows across branches, Turso plus R2 backup.
- **[Yuuko Workspace](https://yuuko-workspace.onrender.com)** — personal tracker from OJT weekends. Timed tasks plus analytics on where the hours went.
- **[Bacaltos Clinic](https://bacaltosclinic.onrender.com)** — capstone platform with scheduling, records, role-based access, and disease forecasting.
- **Internship reps** — inventory and admin systems at Open Space Technology.

Background: fundamentals first in C, Java, and data structures, then web basics and databases, then React with cloud backends, then operating full-stack apps end to end. Solo champion at the Technolympics 2025 Codathon.

### Activity

![snake](https://raw.githubusercontent.com/JohnCliffordAlbarico/JohnCliffordAlbarico/output/github-snake.svg)

### Numbers

<p>
  <img src="https://streak-stats.demolab.com?user=JohnCliffordAlbarico&theme=dark&hide_border=true" alt="streak" />
</p>

<p>
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=JohnCliffordAlbarico&theme=github_dark" alt="summary stats" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=JohnCliffordAlbarico&theme=github_dark" alt="repos per language" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=JohnCliffordAlbarico&theme=github_dark" alt="top language" />
</p>

### Reach me

Email works fastest: cliffordalbarico20@gmail.com — or open an issue on any repo. LinkedIn and Facebook links live on my portfolio.

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0e75b6&height=120&section=footer" alt="footer" />
